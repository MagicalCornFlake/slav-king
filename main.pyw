...
class LootDrop:
    ...
    def __init__(self, pos: tuple[int, int], pickup_amount: int, loot_type: str):
        ...
        self.text = self.current_font.render(
            f"+{self.pickup_amount}", True, [255] * 3)
        self.text_position = [self.hitbox[0] + self.hitbox[2] //
                              2 - self.text.get_width() // 2, self.hitbox[1] - 20]
        self.animation_direction = 1

    def draw(self):
        """Renders the loot drop in the game window."""
        self.current_font = pygame.font.SysFont(
            "comicsans", 19 + self.animation_cycle)
        ...
        v.win.blit(v.bullet_stack if self.loot_type ==
                   "ammo" else v.coin_stack, (self.x_pos, self.y_pos))
        text_to_render = f"+{'$' * int(self.loot_type == 'money')}{self.pickup_amount}"
        self.text = self.current_font.render(
            text_to_render, True, [255] * 3)
...
class Weapon:
    ...
    def __init__(self, pos, name, cost, dmg, rof, full_auto):
        ...
        if self.name == "AK-47":
            ...
            self.hitbox = [self.x_pos + (256 - 192) // 2,
                           self.y_pos + (144 - 104) // 2, 192, 144]
        elif self.name == "Beretta":
            self.hitbox = [self.x_pos + (256 - 101) // 2,
                           self.y_pos + (144 - 72) // 2, 101, 72]
        elif self.name == "Deagle":
            self.hitbox = [self.x_pos + (256 - 128) // 2,
                           self.y_pos + (144 - 128) // 2, 128, 128]
        self.text_position = [self.x_pos + 256 // 2 -
                              self.text.get_width() // 2, self.y_pos + 144 - 10]

    def draw(self):
        """Renders the weapon sprite in the shop menu."""
        if v.selected_gun == self:
            self.text = v.bold_font.render(
                self.name + " - SELECTED", 1, [255] * 3)
        elif self.owned:
            self.text = v.bold_font.render(
                self.name + " - OWNED", 1, [255] * 3)
        else:
            self.text = v.bold_font.render(
...
class AbilityPurchasable:
    ...
    def __init__(self, x_pos, y_pos, name, cost):
        ...
        if self.name == "mayonnaise":
            self.hitbox = [self.x_pos + (256 - 75) // 2,
                           self.y_pos + (144 - 116) // 2, 75, 116]
            self.owned = 0
            self.img = pygame.transform.scale(self.img, (75, 116))
        elif self.name == "beer":
            self.hitbox = [self.x_pos + (256 - 40) // 2,
                           self.y_pos + (144 - 124) // 2, 40, 124]
            self.owned = 0
    ...
    def draw(self):
        ...
        self.owned_text = v.bold_font.render(
            str(self.owned), 1, [255] * 3)
        self.owned_text_position = [self.x_pos + 16, self.y_pos + 12]
        if self.owned > 0:
            pygame.draw.rect(v.win, (72, 240, 112), self.outer_hitbox)
            pygame.draw.rect(v.win, [255] * 3, self.outer_hitbox, 5)
...
class AmmoPurchasable:
    ...
    def __init__(self, x_pos, y_pos, cost):
        ...
        self.cost = cost
        self.text = v.bold_font.render(
            "15 rounds - $" + str(cost), 1, [255] * 3)
        self.owned_text = v.bold_font.render(
            str(v.ammo_count), 1, [255] * 3)
        self.affordable = money_count >= self.cost
        self.flash_sequence = -1
        self.outer_hitbox = [self.x_pos, self.y_pos, 256, 164]
        self.hitbox = [self.x_pos + (256 - 96) // 2,
                       self.y_pos + (144 - 96) // 2, 96, 96]
        self.text_position = [self.x_pos + 256 // 2 -
                              self.text.get_width() // 2, self.y_pos + 144 - 10]
        self.owned_text_position = [self.x_pos + 16, self.y_pos + 12]
...
class Ability:
    ...
    def __init__(self, x_pos, y_pos, name):
        ...
        self.dimensions = [
            x_pos, y_pos, self.text.get_width(), 58 + self.text.get_height()]
        self.bar_dimensions = [self.text_x +
                               self.text.get_width() - 11, self.y_pos + 18, 10, 40]
    ...
    def draw(self):
        ...
        if self.owned > 0 or self.progress > 0:
            ...
            self.owned_text = v.bold_font.render(
                str(self.owned), 1, [255] * 3)
            v.win.blit(v.mayo_jar if self.name ==
                       "mayo" else v.beer_bottle, (self.x_pos, self.y_pos))
        else:
            self.text = v.bold_font.render(self.name, 1, (128, 128, 128))
            self.owned_text = v.bold_font.render("0", 1, (128, 128, 128))
            v.win.blit(v.mayo_jar_bw if self.name ==
                       "mayo" else v.beer_bottle_bw, (self.x_pos, self.y_pos))
        v.win.blit(self.text, (self.text_x, self.y_pos + 58))
        v.win.blit(self.owned_text, (self.text_x + self.text.get_width() -
                   self.dummy_text.get_width() + 5, self.y_pos - 2))
        if self.progress > 0:
            ...
            if self.progress < dims[3]:
                self.bar_fill_dimensions = [
                    dims[0], dims[1] + dims[3] - self.progress, dims[2], self.progress]
            else:
...
class Button:
    ...
    def initialise_dimensions(self):
        ...
        if len(sibling_buttons) == 2:
            ...
            self.dimensions = [x_centre - v.WIN_WIDTH // 6,
                               v.WIN_HEIGHT * 1.75 // 3, v.WIN_WIDTH // 3, 50]
        else:
            ...
            # The height of the button is 50px
            self.dimensions = [v.WIN_WIDTH // 2 -
                               v.WIN_WIDTH // 4, y_centre, v.WIN_WIDTH // 2, 50]
...
class Player(object):
    ...
    walkLeft = []
    for i in range(1, 10):
        walkRight.append(pygame.image.load(
            v.SPRITE_DIR + "R" + str(i) + ".png"))
        walkRight[-1] = pygame.transform.scale(walkRight[-1], (256, 256))
        walkLeft.append(pygame.image.load(
            v.SPRITE_DIR + "L" + str(i) + ".png"))
    ...
    def draw(self):
        ...
        if not self.standing:
            if self.left:
                ...
                v.win.blit(self.walkRight[round(
                    self.walk_count) // 2], (self.x_pos, self.y_pos))
                if not v.paused:
                    self.walk_count += self.vel // 5 / 2
        else:
            if self.right:
...
class Enemy(object):
    ...
    for i in range(1, 12):
        walkRight.append(pygame.image.load(
            v.SPRITE_DIR + "ER" + str(i) + ".png"))
        walkRight[-1] = pygame.transform.scale(walkRight[-1], (256, 256))
        walkLeft.append(pygame.image.load(
            v.SPRITE_DIR + "EL" + str(i) + ".png"))
        walkLeft[-1] = pygame.transform.scale(walkLeft[-1], (256, 256))
    ...
    def draw(self):
        ...
        if self.vel > 0:
            v.win.blit(
                self.walkRight[self.walk_count // 3], (self.x_pos, self.y_pos))
            if not v.paused:
                self.walk_count += 1
        elif self.vel < 0:
            v.win.blit(self.walkLeft[self.walk_count //
                       3], (self.x_pos, self.y_pos))
            if not v.paused:
                self.walk_count += 1
        else:
            self.walk_count = 20
        ...
        self.hitbox = [self.x_pos + 78, self.y_pos + 58, 100, 190]
        # Drawing health bar
        pygame.draw.rect(v.win, (255, 0, 0),
                         (self.hitbox[0], self.hitbox[1] - 35, 100, 20))
        pygame.draw.rect(
    ...
    def hit(self):  # When shot by bullet
        ...
        if self.health - minus > 0:
            ...
            for loot_type in ("ammo", "money"):
                ...
                random_x = v.random.randint(
                    self.x_pos - 10 * variance, self.x_pos + 10 * variance) + 128
                random_y = v.random.randint(
                    self.y_pos - 10 * variance, self.y_pos + 10 * variance) + 128
...
def fire(rapid_fire):
    if not v.firing:
        if rapid_fire:
            v.sounds.append(Effect(
                "bullet_fire_" + v.selected_gun.name + "_auto", channel="gun", should_loop=True))
        elif v.selected_gun.full_auto:
            v.sounds.append(Effect("bullet_fire_" + v.selected_gun.name +
                            "_start", channel="gun", should_loop=False))
        else:
            v.sounds.append(
                Effect("bullet_fire_" + v.selected_gun.name, channel="gun", should_loop=False))
    if v.shot_cooldown_time_passed >= v.shot_cooldown:
        ...
        if slav.left:
            ...
            facing = 1
        # Fires bullet
        v.bullets.append(Projectile((round(slav.x_pos + slav.width // 2),
                                    round(slav.y_pos + slav.height // 2.5)), 3, (0, 0, 0), facing))
...
def redraw_game_window(cop_hovering_over):
    ...
    for bullet in v.bullets:
        bullet.draw()
    score_text = v.bold_font.render(
        "score: " + str(v.score), 1, [255] * 3)
    ...
    fps_counter_text = v.bold_font.render(str(round(fps)) + " FPS", 1, (255 - round(fps / 27 * 255),
                                                                        round(fps / 27 * 255), 0))
    if INFINITE_AMMO:
        ammo_count_text = v.bold_font.render(
            "ammo: infinite", 1, [255] * 3)
    else:
        ammo_count_text = v.bold_font.render(
            "ammo: " + str(v.ammo_count), 1, [255] * 3)
    money_count_text = v.bold_font.render(
    ...
    quit_text = v.big_font.render(
        "Are you sure you want to quit?", 1, [255] * 3)
    for filled_star in range(1, v.wanted_level + 1):
        v.win.blit(v.star_1, (v.WIN_WIDTH - 32 *
                   filled_star - 10, v.WIN_HEIGHT - 32 - 10))
    for empty_star in range(v.wanted_level + 1, 6):
        v.win.blit(v.star_0, (v.WIN_WIDTH - 32 *
                   empty_star - 10, v.WIN_HEIGHT - 32 - 10))
    if v.paused:
        ...
        if v.pause_menu == "shop":
            ...
            v.win.blit(quit_text, (v.WIN_WIDTH // 2 -
                       quit_text.get_width() // 2, v.WIN_HEIGHT // 3))
        else:
            v.win.blit(paused_text_outline, (v.WIN_WIDTH // 2 - paused_text_outline.get_width() // 2,
                                             v.WIN_HEIGHT // 5 + 1))
            v.win.blit(paused_text, (v.WIN_WIDTH // 2 -
                       paused_text.get_width() // 2, v.WIN_HEIGHT // 5))
    ...
    v.win.blit(highscore_text, (v.WIN_WIDTH -
               highscore_text.get_width() - 20, 30))
    v.win.blit(fps_counter_text, (v.WIN_WIDTH -
               fps_counter_text.get_width() - 20, 50))
...
button_volume = Button(
    "music options...", lambda: exec("v.pause_menu = 'volume'"))
button_options_dev = Button("developer options...",
                            lambda: exec("v.pause_menu = 'dev'"))
button_back = Button("back... [ESC]", lambda: exec("v.pause_menu = 'main'"))
slider_volume = Slider("volume: {vol}")
button_mute_music = Button("mute background music",
                           toggle_mute, settings["muted"])
button_back_volume = Button(
    "back... [ESC]", lambda: exec("v.pause_menu = 'options'"))
...
gun_beretta = Weapon((32, v.WIN_HEIGHT // 8), name="Beretta",
                     cost=100, dmg=20, rof=1000, full_auto=False)
gun_deagle = Weapon((v.WIN_WIDTH // 2 - 128, v.WIN_HEIGHT // 8),
                    "Deagle", cost=300, dmg=60, rof=200, full_auto=False)
...
while v.run:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:  # If user clicks red 'x_pos' button to close
            v.paused = True
            v.pause_menu = "quit"
        elif event.type == pygame.KEYDOWN:  # If a key is pressed down this frame
            ...
            # If escape was pressed this frame
                ...
                if v.paused:
                    ...
                    pygame.mixer.pause()
        # Sound effect code
        elif event.type == pygame.KEYUP and event.key == setup.key_index[settings["attack_key"]] and v.firing:
            firing = False
            if v.selected_gun.full_auto:
                pygame.mixer.Channel(1).stop()
                v.sounds.append(
                    Effect("bullet_fire_" + v.selected_gun.name + "_end"))
        ...
        # If a mouse button is pressed this frame
            ...
            if event.button == 1:  # If that mouse button is the left mouse button
                            ...
                            if dim[0] < mouse_pos[0] < dim[0] + dim[2] and dim[1] < mouse_pos[1] < dim[1] + dim[3]:
                                if gun_icon.owned:
                                    ...
                                    if gun_icon.name == "Beretta":
                                        v.purchasables = [
                                            purchasable_light_ammo]
                                    elif gun_icon.name == "Deagle":
                                        v.purchasables = [
                                            purchasable_heavy_ammo]
                                else:
                ...
                # If clicked shop icon
                    ...
                    for purchasable in v.purchasables:
                        purchasable.affordable = money_count >= purchasable.cost
        elif v.slider_engaged and event.type == pygame.MOUSEBUTTONUP and event.button == 1:
    ...
    if pygame.mouse.get_pressed(num_buttons=3)[0] and v.slider_engaged:
        ...
        temp_volume = int(
            (mouse_pos[0] - (v.WIN_WIDTH // 2 - v.WIN_WIDTH / 4)) / (v.WIN_WIDTH // 2) * 100)
    ...
    if not v.paused:
        ...
        if len(v.cops) < v.cop_amount:
            if v.time_passed_since_last_cop_spawned >= v.cop_spawn_delay:
                v.cops.append(Enemy(v.WIN_WIDTH - 1, v.WIN_HEIGHT -
                              v.random.randint(92, 112) - 256 + 64, 256, 256))
        ...
        for fired_bullet in v.bullets:
            ...
            for cop in v.cops:
                if cop.touching_point((fired_bullet.x_pos, fired_bullet.y_pos)):
                    ...
                    break
        # Moving left
        if keys[setup.key_index[settings["left_key"]]] or keys[pygame.K_LEFT]:
            move(-slav.vel)
            slav.left = True
            slav.right = False
            slav.standing = False
        # Moving right
        elif keys[setup.key_index[settings["right_key"]]] or keys[pygame.K_RIGHT]:
            move(slav.vel)
            slav.right = True
            slav.left = False
            slav.standing = False
        ...
        # Jumping
        if not slav.jumping:
            if keys[setup.key_index[settings["jump_key"]]] or keys[pygame.K_UP]:
                slav.jumping = True
